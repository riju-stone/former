...
function ValidationInput({ validation, elementId, formBlockId }: { validation: any; elementId: string; formBlockId: string }) {
    ...
    case "number":
      return (
        <input
          ...
          disabled={false}
          onChange={(e) => updateElementConstraint(elementId, validation.id, Number(e.target.value), formBlockId)}
        />
      );
    case "array":
      return (
        <select
          ...
          onChange={(e) => updateElementConstraint(elementId, validation.id, e.target.value, formBlockId)}
        >
...
function ValidationGroupComponent({ validationGroup, elementId, formBlockId }: { validationGroup: any; elementId: string; formBlockId: string }) {
  return (
